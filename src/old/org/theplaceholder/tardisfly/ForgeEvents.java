...
public class ForgeEvents {
    ...
    @SubscribeEvent
    public static void overrideRenderPlayer(RenderLivingEvent.Pre event){
        if (event.getEntity() instanceof PlayerEntity){
            if (Capabilities.getPlayerTardisFlyCapability((PlayerEntity) event.getEntity()).getTardisExteriorID() != -1) {
                int id = Capabilities.getPlayerTardisFlyCapability((PlayerEntity) event.getEntity()).getTardisExteriorID();
                if (id != -1) {
                    event.setCanceled(true);
                    ExteriorsIDs.render((PlayerEntity) event.getEntity(), event.getMatrixStack(), event.getBuffers(), event.getLight(), event.getPartialRenderTick());
                }
            }
        }
    }
